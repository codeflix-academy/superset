...
describe("loadSetupConfig", () => {
	beforeEach(() => {
		mkdirSync(join(MAIN_REPO, ".studio"), { recursive: true });
	});
	...
	test("loads valid setup config from main repo", () => {
		...
		writeFileSync(
			join(MAIN_REPO, ".studio", "config.json"),
	...
	test("returns null for invalid JSON", () => {
		writeFileSync(join(MAIN_REPO, ".studio", "config.json"), "{ invalid json");
	...
	test("prefers worktree config over main repo config", () => {
		...
		mkdirSync(join(WORKTREE, ".studio"), { recursive: true });
		writeFileSync(
			join(WORKTREE, ".studio", "config.json"),
...
describe("config.local.json", () => {
	...
	test("local config with before/after merges with base setup", () => {
		...
		writeFileSync(
			join(MAIN_REPO, ".studio", "config.local.json"),
			JSON.stringify({
	...
	test("worktree local config takes priority over main repo local config", () => {
		...
		mkdirSync(join(WORKTREE, ".studio"), { recursive: true });
		writeFileSync(
			join(WORKTREE, ".studio", "config.local.json"),
